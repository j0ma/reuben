...
def variance_components_summary(data, config, rounding: int = 2):
    ...
    between_task_sds = (
        data_by_task[score_col].groupby(model_col).std(ddof=1).astype(float)
    )
    ...
    def _mean_sd_min_max(df, prefix):
        return (
            df.agg(["mean", "std", "min", "max"], axis=1)
            .rename(
                columns={
                    "mean": f"{prefix}_mean",
                    "std": f"{prefix}_sd",
    ...
    pieces = [_mean_sd_min_max(repl, "repl_sd")]
    if seed is not None:
        pieces.append(_mean_sd_min_max(seed, "seed_sd"))
    if boot is not None:
        pieces.append(_mean_sd_min_max(boot, "boot_sd"))
    if seed is not None and boot is not None:
        ratio = boot / seed.replace(0, np.nan)
        pieces.append(_mean_sd_min_max(ratio, "boot_to_seed_ratio"))
...
def variance_components_per_task(data, config, rounding: int = 2) -> pd.DataFrame:
    (
        task_col,
        model_col,
        score_col,
    ...
    score = data_by_task[score_col].unstack(task_col)
    repl = data_by_task[repl_sd_col].unstack(task_col)
    seed = data_by_task[seed_sd_col].unstack(task_col) if seed_sd_col else None
    boot = data_by_task[boot_sd_col].unstack(task_col) if boot_sd_col else None
    ...
    for m in score.index:
        for t in score.columns:
            rec = {
                model_col: m,
                task_col: t,
                score_col: float(score.loc[m, t]),
                repl_sd_col: float(repl.loc[m, t]),
            }
            if seed is not None:
                rec[seed_sd_col] = float(seed.loc[m, t])
            if boot is not None:
                rec[boot_sd_col] = float(boot.loc[m, t])
            records.append(rec)
...
def compute_all_statistics(leaderboard, orig_results, uniq_models, config):
    ...
    for agg_name, agg_results in stats["agg_over_task"]["raw"].items():
        ...
        diffs_origscore_lowertrimask = np.tril_indices_from(diffs_origscore, k=0)
        diffs_origscore_lowertrinan = diffs_origscore.copy()
        diffs_origscore_lowertrinan[diffs_origscore_lowertrimask] = np.nan

        diffs_sd_lowertrimask = np.tril_indices_from(diffs_sd, k=0)
        diffs_sd_lowertrinan = diffs_sd.copy()
        diffs_sd_lowertrinan[diffs_sd_lowertrimask] = np.nan

        diffs_effect_size = (diffs_origscore_lowertrinan / diffs_sd_lowertrinan).round(
            config.rounding
        )
...
def compute_pairwise_diff_statistics(
    ...
    for i in range(num_models - 1):
        for j in range(i + 1, num_models):
            ...
            within_sds = diff_rep.std(axis=1, ddof=1)
            within_sds[within_sds == 0] = np.nan

            between_sd = float(diff_obs.std(ddof=1))
            ...
            std_err_mean = float(sd_pred / math.sqrt(num_tasks))
