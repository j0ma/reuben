...
AGGREGATOR_REGISTRY = {}
AGGREGATOR_PRETTYNAME_REGISTRY: dict[str, str] = {
...
def register_aggregator(name: str, pretty_name: Optional[str] = None):
    def actual_register_method(f):
        AGGREGATOR_REGISTRY[name] = f
        if (
            isinstance(pretty_name, str)
            and isinstance(name, str)
            and name not in AGGREGATOR_PRETTYNAME_REGISTRY
        ):
